...
class WanYiYun():
    def __init__(self):
        self.playlist_url='https://music.163.com/playlist?id=3220130759'#歌单地址
...
#由于歌单内容是通过JS生成的，所以此处运用requests_html这个库来实现JS渲染从而获得歌单
    ...
    def download(self):
        ...
        for music in music_list:
            ...
            comurl='https://music.163.com/weapi/v1/resource/comments/R_SO_4_'+music_id+'?csrf_token='
            # print(comurl)
            res=requests.post(comurl,headers=self.headers, data=formdata)
            # print(res.json())
            for comments in res.json()['hotComments']:
